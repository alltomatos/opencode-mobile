...
Não existe endpoint de "criar projeto"/"mkdir"/"clone" dedicado no backend — o mecanismo real é: cria uma sessão de bootstrap ancorada em `/home/opencode` (o home do usuário do servidor, que sempre existe — evita o problema de galinha-e-ovo de `/home/opencode/projects` ainda não existir na primeira vez) e roda o comando via `POST /session/:id/shell`. `/home/opencode/projects` como raiz padrão é uma convenção deste app mobile, não uma configuração do servidor — outros clientes (desktop, TUI) não a seguem.

**Listagem de projetos: `GET /file`, não `GET /project`.** Versão anterior deste documento descrevia registrar cada pasta nova como "projeto" via `GET /project/current`/`POST /project/git/init`. Isso foi abandonado depois de uma investigação ao vivo (ver commit "Fix projects vanishing") revelar que a resolução de "diretório → projeto" do servidor é cacheada **pra sempre** por processo, só reconhece pastas com git, e mesmo `initGit` não corrige uma pasta já cacheada errado. Pior: confirmamos ao vivo que projetos reais já criados pelo usuário (pastas físicas existentes) simplesmente não apareciam em `GET /project` — não era só um problema de pasta nova, o endpoint escondia projetos existentes também. A solução (sugestão do usuário, correta): listar as subpastas reais de `/home/opencode/projects` via `GET /file?directory=/home/opencode/projects&path=.`, filtrando `type: "directory"`. Isso não depende de git nem de nenhuma resolução de projeto do servidor — uma pasta sem git aparece igual a uma com. Sessões continuam agrupadas por comparação direta da string `directory`, nunca por id de projeto do servidor.
