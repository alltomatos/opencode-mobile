...
3. **Modo de execução (Manual / Aceitar edições / Planejar / Automático).** Este conceito **não existe hoje no backend deste fork** — não há endpoint nem flag de "auto-aprovar tudo" no servidor (confirmado por busca no código; permissão é sempre por regra `PermissionRuleset` no agente, não um modo de sessão). A abordagem que **não exige mudança de backend**: o "Automático" no mobile é um comportamento **só do cliente** — quando ligado, o app responde sozinho `always` a todo `permission.asked` que chegar (mesmo fluxo de `POST /permission/:id/reply` já implementado, só automatizando a escolha em vez de mostrar o card). "Manual" é o comportamento atual (sempre mostra o card). "Aceitar edições" e "Planejar" ficam como modos futuros — dependem de saber diferenciar o tipo de permissão pedida (edição de arquivo vs. execução de comando vs. outra coisa), o que o payload de `permission.asked` (`permission`, `patterns`) já dá pra fazer, mas ainda não foi mapeado.
